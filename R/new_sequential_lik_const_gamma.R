#' Sequential likelihood logic (separate mutation rates) but with constant selection coefficient
#' Only used to conduct LRT to test whether new sequential likelihood model with distinct selection 
#' fits better than constant gamma.
#' 
...
sequential_lik_dev_const_gamma <- function(rates_tumors_with, rates_tumors_without,
                                           sample_index, sequential_mut_prop) {
  ...
  fn = function(gamma) {
    ...
    # expand scalar gamma to all stages
    gamma_vec = rep.int(gamma, num_pars)
    
    # samples without mutation
    sum_log_lik = -1 * sum(mapply(
      function(rate, stage) {
        flux = gamma_vec[1:stage] * rate * sequential_mut_prop[1:stage]
        sum(flux)
      },
    ...
    if (length(rates_tumors_with) > 0) {
      sum_log_lik = sum_log_lik + sum(mapply(
        function(rate, stage) {
          lik_no_mutation = exp(-1 * gamma_vec * rate * sequential_mut_prop)
          lik_mutation = 1 - lik_no_mutation
          
          cum_lik_no_mut = c(1, cumprod(lik_no_mutation))
          log(sum(cum_lik_no_mut[1:stage] * lik_mutation[1:stage]))
        },
        rates_tumors_with, stages_tumors_with
      ))
    }
    
    if (!is.finite(sum_log_lik)) return(-1e200)
    return(-1 * sum_log_lik) # negative loglikelihood
  }
  
  # one starting value (scalar)
  formals(fn)[["gamma"]] = 1000
  
  # single parameter name
  bbmle::parnames(fn) = "gamma_const"
  return(fn)
}